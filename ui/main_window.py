...
class _StdoutSignaler(QObject):
    """Живёт в GUI-потоке, принимает строки из чужих потоков через сигнал."""
    line = Signal(str)


class StreamRedirector(StringIO):
    """Thread-safe редирект stdout/stderr: пишет в Qt-сигнал."""

    def __init__(self, signaler: _StdoutSignaler, mirror=None):
        super().__init__()
        self._signaler = signaler
        self._mirror = mirror  # оригинальный stdout, если хочется дублировать в консоль
...
class JarvisWindow(QMainWindow):
    def __init__(self):
        ...
        # bridge — все события через Qt-сигналы
        ...
        # Signaler для редиректа stdout/stderr — тоже thread-safe
    ...
    def build_ui(self):
        ...
        self.info_view.setPlainText(
            "voice / ui / Jarvis\n"
            "TTS — streaming, STT — Whisper, LLM — Ollama.\n"
            "GUI показывает диалог и системные логи.\n"
            "Дальше можно добавить waveform, настройки и т.п."
        )
    ...
    # ====== управление ассистентом через bridge ======
    ...
    def start_assistant(self):
        ...
        # Thread-safe редирект: print(...) из чужих потоков будет эмитить сигнал,
        # слот on_stdout_line получит текст уже в GUI-потоке.
    ...
    # ====== слот для stdout/stderr редиректа (GUI-поток) ======

    def on_stdout_line(self, text: str):
        # печать уже происходит в консоль через mirror; здесь только GUI
        if text is None:
            return
        s = str(text).rstrip("\r\n")
        if not s:
