        ...
        for ch in (t.get("checks") or []):
            ...
            if ctype == "file_exists":
                ...
                if rel and rel not in output_paths and _is_input_fixture(rel, deliverables, output_paths):
                    ...
                    continue
            new_checks.append(ch)
        # P11.7 FIX-4: если после фильтрации список checks пустой —
        # добавляем rc_zero как минимальный sanity-check.
...
def _heal_via_aider(slug: str, plan: dict, failed: dict, spec: dict | None = None) -> dict:
    ...
    try:
        pdir = project_dir(slug)
        # P11.7 FIX-1: передаём read_only_files (контракты соседей) в heal
...
def _heal_loop(slug: str, spec: dict, plan: dict, test_results: list[dict], budget: Budget) -> list[dict]:
    ...
    import hashlib as hashlib  # уже импортирован через hashlib выше, но явно для ясности
    ...
    for heal_iter in range(1, MAX_HEAL_ITERS + 1):
        ...
        if diag is None and AIDER_ENABLED and aider_runner.is_aider_available(AIDER_BIN):
            try:
                ...
                break
            if ah.get("ok"):
                # P11.7 FIX-2: no-change guard — если aider не изменил файл, не тратим ретест
                ...
                if _ah_target:
                    try:
                        ...
                        # Сохраняем хеш ДО в _pre_hashes (инициализируем выше при необходимости)
                        _pre = _pre_hashes.get(_ah_target)
                        if _pre is not None and _pre == _hash_after:
                            _changed = False
                            logger.info(f"[heal.no_change] aider claimed ok but {_ah_target} unchanged — skipping retest")
                            add_phase(slug, f"heal:iter{heal_iter}", "failed",
    ...
    try:
        ...
        if 3 >= skip_until:
            try:
                ...
                # P11.7 FIX-5: BudgetExceeded в test отдельно — не роняем в общий except
