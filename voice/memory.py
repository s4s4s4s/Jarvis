"""
voice/memory.py — Долгосрочная память Jarvis

Факты хранятся в JSON-файле:
C:/jarvis/memory/facts.json
...
# Для извлечения фактов используем fast model
...
def _is_duplicate(new_fact: str, existing: list[dict], threshold: float = 0.7) -> bool:
    """Простая проверка дубликата — нормализованное слово-пересечение."""
...
Тебе дан один ход диалога между пользователем и ассистентом.
Твоя задача: найти ТОЛЬКО конкретные, долгосрочно полезные факты о пользователе или его жизни.

Правила:
1. Выводи ТОЛЬКО факты о пользователе (его имя, возраст, привычки, предпочтения, работа, планы, здоровье, интересы, важные события).
